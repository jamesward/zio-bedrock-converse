...
/**
 * Integration tests: each scenario from [[SharedSpec]] runs against the
 * live Bedrock service. Gated by `AWS_BEARER_TOKEN_BEDROCK`; skipped
 * when the env var isn't set.
 *
 * Optional env overrides:
 *  - `BEDROCK_TEST_MODEL_ID` — defaults to a recent Claude Sonnet 4.5
 *    inference profile.
...
object IntegrationSpec extends ZIOSpecDefault:
  ...
  private val testLayer: ZLayer[Client, Nothing, BedrockConverse] =
    BedrockConverse.layer(testApiKey, testRegion, testModelId)

  private def asTest(s: SharedSpec.Scenario): Spec[BedrockConverse, Any] =
    test(s.name)(s.run)

  def spec = suite("Integration")(SharedSpec.all.map(asTest)*)
